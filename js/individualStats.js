...
function displayResultPlayer(data) {
    ...
    $.each(data.results.bindings, function(index, bs) {
        ...
        player.innerHTML = `<div id='name2'>Nom : `+bs.n.value+`</div><br>
                            ...
                            <button type="button" id="addDbpedia" onclick="httpGetAsync()">Étendre avec dbpedia</button><br>
        `;
...
function httpGetAsync() {
    ...
    SELECT ?x ?y
    WHERE {
        dbr:`+window.subject_player+` dbo:birthPlace ?x;
        dbo:birthDate ?y
      }`;
    var theUrl = url + "?query=" + encodeURIComponent(query);
    ...
    $.ajax({
        ...
        dataType: 'json',
        success: function(data) {
            var player_dbpedia = document.createElement("div");
            if (data.results.bindings.length == 0) {
              player_dbpedia.id = "player3";
              player_dbpedia.innerHTML = "<br><div>Nous n'avons pas pu trouver ce joueur dans dbpedia.</div>";
              $("#player_info").append(player_dbpedia);
            }
            else {
              $.each(data.results.bindings, function(index, bs) {
                player_dbpedia.id = "player3";
                let birthPlace = bs.x.value.split("http://dbpedia.org/resource/");
                birthPlace = birthPlace[1].split("_").join(" ");
                let birthDate = bs.y.value;
                player_dbpedia.innerHTML = `<br><div>Lieu de naissance :<br> `+birthPlace+`</div><br>
                <div>Date de naissance : `+birthDate+`</div>`;
                $("#player_info").append(player_dbpedia);
              });
            }
        },
